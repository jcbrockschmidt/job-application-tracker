...
- **AI usage display:** An info bar below the Master CV page header shows AI usage in the format `24h: $<AMOUNT> • Last: $<AMOUNT> • model`. The "24h" amount tracks the global rolling 24-hour estimated spend total (turning orange if the configured limit is exceeded); "Last" shows the estimated cost of the most recent AI operation; and "model" displays the currently active Claude model. Detailed token counts for the last operation are available via tooltip.
...
**AI usage display:** An info bar below the view header shows AI usage in the format `24h: $<AMOUNT> • Last: $<AMOUNT> • model`, consistent with the Master CV view.
...
- A **New Session** button creates a session, prompting the user to paste a job description. Submitting it analyzes the job description (company/role extraction) and creates the session record. The dialog closes as soon as the session is created — the user is not navigated away and can continue using the app freely while resume generation runs in the background.
- Sessions are listed in a persistent **sidebar** on the left of the main window. Each sidebar entry shows the company name, job title, and a status badge (Draft / Final). While a session's resume is being generated for the first time, the sidebar entry shows a spinner and "Generating…" in place of the role title and status badge, and the entry is **not clickable**. Once generation completes the entry becomes interactive and shows the Draft badge. The active session is highlighted; clicking any ready entry opens that session.
...
- **AI usage display** is shown on the right side of the session tab bar and in a dedicated info bar on the Master CV and Writing Profile pages. Usage is displayed in the format `24h: $<AMOUNT> • Last: $<AMOUNT> • model`. This captures the global rolling 24-hour estimated spend ("24h"), the cost of the most recent operation ("Last"), and the active Claude model. Detailed token counts are available in tooltips. All costs are estimates based on published model pricing.
- **Spending limit** — the app logs each AI operation with a timestamp, model, input tokens, output tokens, and estimated cost. The global rolling 24-hour spend total is computed from log entries within the last 24 hours; entries older than 24 hours are excluded automatically. This log is persisted to the SQLite database (`spend_log` table) and loaded on app launch, so the rolling window remains accurate across restarts and tracks usage across the entire app. When the total exceeds the user-configured limit, a persistent warning banner appears in the session and Master CV views. Any subsequent generation attempt shows a **spending limit warning dialog** before proceeding, displaying the 24-hour estimated spend vs. the limit with options to cancel or generate anyway. The limit can be set to 0 to disable warnings.
