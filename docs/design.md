...
- **AI usage display:** An info bar below the Master CV page header shows the model currently in use, the input and output token counts for the last AI operation (e.g. a Regenerate run), and an estimated cost. To track total usage across the entire app, the global rolling 24-hour estimated spend total is also shown (labeled as "24h spend") alongside the configured limit; it turns orange when the limit is exceeded.
...
**AI usage display:** An info bar below the view header shows the model, last-operation token counts, and estimated cost, as well as the global rolling 24-hour estimated spend, consistent with the Master CV view.
...
- The **session view** is structured as follows:
  ...
  - **Tab bar** — four tabs organize session content: **Resume**, **Cover Letter**, **Match Report**, and **Description**. Token usage for the last AI operation is shown on the right side of the tab bar (model name, input and output token counts, estimated cost). The global rolling 24-hour estimated spend total (labeled "24h spend") is also shown here to track usage across the entire app. When this spend exceeds the configured limit, an orange badge showing the current vs. limit amounts appears to the left of the token usage, and an amber warning bar is shown below the tab bar across all tabs.
...
- **Token usage** is displayed after each AI operation in the right side of the session tab bar and in a dedicated info bar on the Master CV and Writing Profile pages. This display shows the model name, input and output token counts, and an estimated cost for the last AI operation. To track total usage across the entire app, the global rolling 24-hour estimated spend is also shown (labeled "24h spend"). Cost is labeled as an estimate since pricing may change.
